...
class SnapOperatorTest(unittest.TestCase):
  ...
  def setUp(self) -> None:
    ...
    self._exit_stack.enter_context(
        mock.patch.object(snap_holder.Snapshot, 'create_from',
                          self._mock_create_from))

    def fake_get_deletes(
        self, now: datetime.datetime, records: list[tuple[datetime.datetime,
                                                          str]]
    ...
    self._exit_stack.enter_context(
        mock.patch.object(deletion_logic.DeleteManager, 'get_deletes',
                          fake_get_deletes))
    self._exit_stack.enter_context(
        mock.patch.object(snap_operator, '_get_old_backups',
                          lambda configs: self._old_snaps))
