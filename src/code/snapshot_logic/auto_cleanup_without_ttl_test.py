...
def _deleted_times(rules: dict[int, int], records: list[int]):
    ...
    return [int(0.5 + x.timestamp()) - nowts for x, _ in mgr.get_deletes(now, toscan)]
...
class TestDeletionLogic(unittest.TestCase):
    ...
    def test_multi_rule(self):
        self.assertEqual(
            _deleted_times({100: 0, 10: 0}, [-115, -105, -25, -15, -5]),
            [-115, -105, -25, -15, -5],
        )
        self.assertEqual(
            _deleted_times({100: 0, 10: 2}, [-115, -105, -25, -15, -5]),
            [-115, -105, -25],
        )
        self.assertEqual(
            _deleted_times({100: 0, 10: 3}, [-115, -105, -25, -15, -5]), [-115, -105]
        )
        self.assertEqual(
            _deleted_times({100: 0, 10: 4}, [-115, -105, -25, -15, -5]), [-115, -105]
        )
        self.assertEqual(
            _deleted_times({100: 0, 10: 40}, [-115, -105, -25, -15, -5]), []
        )

        self.assertEqual(
            _deleted_times({100: 2, 10: 0}, [-115, -25, -15, -5]), [-15, -5]
        )
